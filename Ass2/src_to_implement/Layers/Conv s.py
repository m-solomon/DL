...
class Conv:
    def __init__(self, stride_shape, convolution_shape , num_kernels):
        ...
        #weights shape = (num_kernels(H), ker_Nr_channels,  , ker_m, ker_n)
    ...
    def initialize(self, weights_initializer, bias_initializer):
        self.weights = weights_initializer.initialize(self.weights_shape, 
                                                      self.c_shape[0] * self.c_shape[1] * self.c_shape[2] , 
                                                      self.num_kernels * self.c_shape[1]*self.c_shape[2])
    ...
    def forward(self,input_tensor): #input tensor shape (b,c,y) or (b,c,y,x)
        ...
        for b in range(self.input_tensor.shape[0]):
            ...
            #shape = (in_Nr_channels, y_in/strid, x_in/stride)

            correlated = np.zeros((self.num_kernels, output_shape[1], output_shape[2]))
            #correlated shape = (num_kernels, y_in/strid, x_in/stride) 
            ...
            for i in range(self.num_kernels):
                ...
                #corr_output_shape = (in_y, in_x)
                correlated[i] = temp[self.input_tensor.shape[1] // 2][::int(self.s1), ::int(self.s2)] + self.bias[i]
                #corr_output_stided_shape = (in_Nr_channels, in_y/strid, in_x/strid)

            stacked_convolved.append(correlated)

        stacked_convolved_npArray = np.asarray(stacked_convolved)
        #corr_output_stided_shape_all_input = (in_batch_size, in_Nr_channels, in_y/strid, in_x/strid)
