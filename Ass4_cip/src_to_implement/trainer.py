...
class Trainer:
    ...
    def train_step(self, x, y):
        ...
        x = self._model(x)        # propagate through the network
        #sig_obj = t.nn.Sigmoid()  # sigmoid here instead of in model.py
        #x = sig_obj(x)
        loss = self._crit(x, y)   # calculate the loss
    ...
    def val_test_step(self, x, y):
        ...
        ###################################################################################################
        ###################################################################################################
        predict = self._model(x)          # propagate through the network
        #sig_obj = t.nn.Sigmoid()         # sigmoid here instead of in model.py
        #predict = sig_obj(predict)       # predict:
        predict = t.round(predict).float()# calculate the predictions
        loss = self._crit(predict , y)    # calculate the loss
        return loss, predict              # return the loss and the predictions
        ###################################################################################################
        ###################################################################################################
