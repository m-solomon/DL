...
class ChallengeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode_flag == "train":
            ...
            label = self.train_labels.iloc[idx].as_matrix() #debug
        else:
            img_name = self.d_val.iloc[idx]
            label = self.val_labels.iloc[idx] #debug

        img_path = os.path.join("/Users/Hagag/PycharmProjects/dl/", img_name)
...
def get_train_dataset():
    ...
    # instantiate
    train_data = ChallengeDataset("train", "/Users/Hagag/PycharmProjects/dl/src_to_implement/train.csv",  0.33,  transform = transform_train)
...
def get_validation_dataset():
    ...
    transform_validation = tv.transforms.Compose([tv.transforms.ToPILImage(),
                                                  tv.transforms.ToTensor(),
                                                  tv.transforms.Normalize(train_mean, train_std)])
    validation_data = ChallengeDataset("val", "/Users/Hagag/PycharmProjects/dl/src_to_implement/train.csv",  0.33,  transform = transform_validation)
